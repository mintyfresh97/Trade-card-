import streamlit as st import requests import os from datetime import datetime from PIL import Image, ImageDraw, ImageFont import io
...
Mapping from full asset name to ticker (used for precision & icon)

ticker_map = { 'Bitcoin (BTC)': 'BTC', 'Ethereum (ETH)': 'ETH', 'XRP (XRP)': 'XRP', 'Solana (SOL)': 'SOL', 'Cardano (ADA)': 'ADA', 'Chainlink (LINK)': 'LINK', 'Curve (CRV)': 'CRV', 'Convex (CVX)': 'CVX', 'Sui (SUI)': 'SUI', 'Fartcoin (FARTCOIN)': 'FARTCOIN', 'Ondo (ONDO)': 'ONDO' }
...
def get_crypto_price_from_coingecko(asset_name): try: coin_id = coingecko_ids.get(asset_name) if not coin_id: return None url = f"https://api.coingecko.com/api/v3/simple/price?ids={coin_id}&vs_currencies=usd" res = requests.get(url, timeout=5) data = res.json() return data[coin_id]['usd'] except: return None
...
with col1: full_asset_name = st.selectbox("Select Asset", list(coingecko_ids.keys())) asset = ticker_map[full_asset_name] icon_path = f"assets/{icon_map.get(asset, '')}" if os.path.exists(icon_path): st.image(icon_path, width=32) else: st.warning("Icon not found")

position = st.number_input("Position Size (£)", value=500.0) leverage = st.number_input("Leverage", value=20)

--- Live price ---

live_price = get_crypto_price_from_coingecko(full_asset_name) if live_price: entry = st.number_input("Entry Price", value=float(live_price)) else: entry = st.number_input("Entry Price", value=0.0) st.warning("Live price not available")

stop_loss = st.number_input("Stop Loss", value=0.0) take_profit = st.number_input("Take Profit", value=0.0) trade_date = datetime.now().strftime("%Y-%m-%d")

--- Calculations ---

digits = precision_map[asset] total_exposure = position * leverage risk = abs(entry - stop_loss) * total_exposure / entry if entry != 0 else 0 reward = abs(take_profit - entry) * total_exposure / entry if entry != 0 else 0 breakeven = round((entry + stop_loss) / 2, digits) rr_ratio = round(reward / risk, 2) if risk != 0 else 0
