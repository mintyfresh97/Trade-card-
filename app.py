...
def asset_data_mode():
    st.title("Asset Data")
    # [Paste your full asset_data_mode implementation here]
    pass


def trade_journal_mode():
    st.title("Trade Journal & Checklist")
    # [Paste your full trade_journal_mode implementation here]
    pass


def mindset_mode():
    st.title("Mindset Dashboard")
    # [Paste your full mindset_mode implementation here]
    pass
...
def strategy_mode():
    ...
    df_hist = pd.read_sql(
        "SELECT date AS 'Date', asset AS 'Asset', strategy AS 'Strategy', rr_ratio AS 'RR Ratio', outcome AS 'Outcome', notes AS 'Notes' FROM trade_log ORDER BY date",
        conn
    )
    if not df_hist.empty:
        st.markdown("### Trade History")
        st.dataframe(df_hist)
        # Sidebar filters and analytics as before
    else:
